...
def test_basic_init(mocker):
    ...
    ThemeSet([], theme_mock, setting_mock, "1.0.0")


def test_list_themes(mocker):
    def_mock = mocker.Mock()
    def_mock.configure_mock(
        version=lambda: Version("1.0.0"),
        name="default",
        themedir="",
    )
...
def test_theme_call_order(mocker):
    ...
    # Try mock first, then fallback to default
    test_run([theme_mock, def_mock])
