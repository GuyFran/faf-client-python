...
class IrcUserset(ModelItemSet[str, IrcUser]):
    ...
    def del_item(self, key: str, *, _transaction: ModelTransaction = ModelTransaction()) -> None:
        user = super().del_item(key, _transaction=_transaction)
        if user is None:
            return
    ...
    def _at_user_updated(
        self,
        user: IrcUser,
        olduser: IrcUser, _transaction: ModelTransaction = ModelTransaction(),
    ) -> None:
